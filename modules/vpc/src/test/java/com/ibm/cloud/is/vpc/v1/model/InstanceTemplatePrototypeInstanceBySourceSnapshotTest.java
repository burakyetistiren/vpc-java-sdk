...
/**
 * Unit test class for the InstanceTemplatePrototypeInstanceBySourceSnapshot model.
 */
public class InstanceTemplatePrototypeInstanceBySourceSnapshotTest {
  ...
  public void testInstanceTemplatePrototypeInstanceBySourceSnapshot() throws Throwable {
    ...
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModel.name(), "my-instance");
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModel.networkInterfaces(), java.util.Arrays.asList(networkInterfacePrototypeModel));
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModel.placementTarget(), instancePlacementTargetPrototypeModel);
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModel.profile(), instanceProfileIdentityModel);
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModel.resourceGroup(), resourceGroupIdentityModel);
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModel.totalVolumeBandwidth(), Long.valueOf("500"));
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModel.userData(), "testString");
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModel.volumeAttachments(), java.util.Arrays.asList(volumeAttachmentPrototypeInstanceContextModel));
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModel.vpc(), vpcIdentityModel);
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModel.bootVolumeAttachment(), volumeAttachmentPrototypeInstanceBySourceSnapshotContextModel);
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModel.primaryNetworkInterface(), networkInterfacePrototypeModel);
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModel.zone(), zoneIdentityModel);

    String json = TestUtilities.serialize(instanceTemplatePrototypeInstanceBySourceSnapshotModel);
    ...
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModelNew.placementTarget().toString(), instancePlacementTargetPrototypeModel.toString());
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModelNew.profile().toString(), instanceProfileIdentityModel.toString());
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModelNew.resourceGroup().toString(), resourceGroupIdentityModel.toString());
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModelNew.totalVolumeBandwidth(), Long.valueOf("500"));
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModelNew.userData(), "testString");
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModelNew.vpc().toString(), vpcIdentityModel.toString());
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModelNew.bootVolumeAttachment().toString(), volumeAttachmentPrototypeInstanceBySourceSnapshotContextModel.toString());
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModelNew.primaryNetworkInterface().toString(), networkInterfacePrototypeModel.toString());
    assertEquals(instanceTemplatePrototypeInstanceBySourceSnapshotModelNew.zone().toString(), zoneIdentityModel.toString());
  }

  @Test(expectedExceptions = IllegalArgumentException.class)
  public void testInstanceTemplatePrototypeInstanceBySourceSnapshotError() throws Throwable {
    new InstanceTemplatePrototypeInstanceBySourceSnapshot.Builder().build();
  }
